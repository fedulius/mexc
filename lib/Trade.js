...
class Trade {
  ...
  constructor(client, tradeSettings) {
    this.#client = client;
    this.#buyPercent = tradeSettings.buyPercent;
    this.#sellPercent = tradeSettings.sellPercent;
    this.#emergencyExit = tradeSettings.emergencyExit;
  }
  ...
  hardTrade(symbol) {
    ...
    if (parseFloat(currPrice.price) > lowPrice && parseFloat(currPrice.price) <= buyPrice ) {
      console.log(this.openOrder(symbol, 'BUY', 'MARKET', {
        quantity: 10
      }));
    }
  ...
  getPriceEndpoint(lowPrice, highPrice) {
    ...
    const buyDiffer = differ * this.#buyPercent;
    const sellDiffer = differ * this.#sellPercent
